...
def run_pipeline():
    ...
    # -------------------------------------------------------------
    # 2. Historique (CORRECTION FINALE AVEC TOUS LES FIX)
    # -------------------------------------------------------------
    ...
    print(f"   📁 Fichier cible: {history_file}")

    # Préparation des données de prix
    daily_prices = df_daily['adj close'].unstack().ffill()

    # 🔧 TÉLÉCHARGEMENT INDICE CAC40 OFFICIEL (VERSION FINALE)
    print("   📊 Téléchargement indice CAC40 officiel (^FCHI)...")
    start_bench = df_daily.index.get_level_values('date').min()
    # 🔧 FIX 1: +1 jour pour inclure aujourd'hui
    end_bench = df_daily.index.get_level_values('date').max() + pd.DateOffset(days=1)

    try:
        cac40_data = yf.download('^FCHI', 
                                 start=start_bench, 
                                 end=end_bench, 
                                 progress=False,
                                 auto_adjust=False)

        # 🔧 FIX 2: Gérer les colonnes MultiIndex ET utiliser 'Close' pour les indices
        if not cac40_data.empty:
            # Vérifier si les colonnes sont MultiIndex
            if isinstance(cac40_data.columns, pd.MultiIndex):
                # Format: [('Close', '^FCHI'), ('High', '^FCHI'), ...]
                # Pour les indices, utiliser 'Close' (pas de 'Adj Close')
                if 'Close' in cac40_data.columns.get_level_values(0):
                    adj_close = cac40_data['Close'].iloc[:, 0]
                elif 'Adj Close' in cac40_data.columns.get_level_values(0):
                    adj_close = cac40_data['Adj Close'].iloc[:, 0]
                else:
                    raise Exception("Ni Close ni Adj Close trouvé")
            else:
                # Colonnes simples: essayer Close puis Adj Close
                if 'Close' in cac40_data.columns:
                    adj_close = cac40_data['Close']
                elif 'Adj Close' in cac40_data.columns:
                    adj_close = cac40_data['Adj Close']
                else:
                    raise Exception("Colonne de prix introuvable")

            market_returns = adj_close.pct_change()
            print(f"   ✓ CAC40 officiel téléchargé ({len(market_returns)} jours)")
        else:
            raise Exception("Données CAC40 vides")

    except Exception as e:
        print(f"   ⚠️ Erreur CAC40: {e}")
        print(f"   → Utilisation fallback (moyenne robuste)...")
        # Fallback vers méthode robuste avec filtrage outliers
        raw_returns = daily_prices.pct_change()
        clean_returns = raw_returns[raw_returns.abs() < 0.20]
        market_returns = clean_returns.mean(axis=1)
    ...
    if os.path.exists(history_file):
        try:
            ...
            print(f"   ⚠️ Erreur lecture historique ({e}), création nouveau.")
    ...
    if new_dates.empty:
        print(f"   ✓ Historique à jour (dernière date: {last_recorded_date.date()})")
    else:
        ...
        for date in new_dates.index:
            ...
            # Affichage limité pour ne pas spammer la console
            if len(new_rows) <= 5 or len(new_rows) % 100 == 0:
        ...
        if new_rows:
            ...
            print(f"   ✓ portfolio_history.csv sauvegardé ({len(hist_df)} jours)")
            print(f"   📍 Emplacement: {history_file}")
            ...
            if os.path.exists(history_file):
                ...
                print(f"   ❌ ATTENTION: Fichier non trouvé après sauvegarde!")
    ...
    with open(debug_file, "w") as f:
        ...
        f.write(f"History exists: {os.path.exists(history_file)}\n")
        if os.path.exists(history_file):
            f.write(f"History size: {os.path.getsize(history_file)} bytes\n")
